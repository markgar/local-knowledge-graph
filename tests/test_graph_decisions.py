...
def assert_parity(env, selection):
    ...
    for member in selection.members:
        ...
        assert member.relationship_ids == tuple(
            sorted(
        ...
        for identifier in member.relationship_ids:
            ...
            assert (
                tuple(e.captured.reference for e in relation.support)
...
def test_actual_native_join_and_full_proofs(tmp_path):
    ...
    with LocalGraphSession(
        ...
        assert (
            GraphRelationshipDecisionsResult.model_validate_json(
        ...
        for assertion in (
            selection.members[0].decision,
            selection.relationships[0].assertion,
        ):
            for evidence in assertion.support:
                capture = evidence.captured
                view = env.evidence.citation(
                    env.scope,
                    StoredCitation(
...
def test_realistic_1001_capacity(tmp_path, monkeypatch, mixed_revisions):
    ...
    with LocalGraphSession(
        ...
        for measurement, ctx in zip(measurements, contexts, strict=True):
            ...
            measurement.update(
                peak_scratch=budget._scratch_peak,
                final_scratch=budget._scratch,
                visits=budget._visits,
                vm=budget._vm,
            )
...
def test_ambiguity_empty_alias_preflight_and_no_traversal_loop(native, monkeypatch):
    ...
    absent = graph.relationship_decisions(
        request(
            env,
            start=GraphEntitySelector(entity_id="absent"),
        )
    )
    ...
    for predicate in ("unknown", "work:decision"):
        failed = graph.relationship_decisions(
            request(
                env,
                predicate=predicate,
                start=GraphEntitySelector(entity_id="absent"),
    ...
    assert (
        graph.relationship_decisions(
            request(
                env,
                start=GraphEntitySelector(name="Exact alias"),
            )
        ).count
        == 12
    )
...
def test_incoming_reached_endpoint_and_unrelated_decisions(tmp_path):
    ...
    edges = write(
        ...
        tuple(
            AddAssertion(
                ...
                object=EntityObject(
                    kind="entity", entity=StoredEntity(kind="stored", entity_id=root)
                ),
                interpretation="explicit" if i < 2 else "inferred",
                support=env.support,
            )
            for i in range(3)
        ),
    ...
    with LocalGraphSession(
        ...
        req = GraphRelationshipDecisionsRequest(
            request_id="incoming",
            scope=env.scope,
            start=GraphEntitySelector(entity_id=root),
            predicate="related",
            direction="incoming",
        )
        ...
        outgoing = graph.relationship_decisions(
            req.model_copy(
                update={
                    "direction": "outgoing",
                    "start": GraphEntitySelector(entity_id=reached),
                }
            )
        )
...
def test_withdrawal_exact_current_history_refresh_and_independent_paths(native, monkeypatch):
    ...
    assert all(
        m.relationship_ids == (last,) for m in one_edge.members if m.decision.subject_id == project
    )
...
@pytest.mark.parametrize(
    "mutation",
    ["decision-id", "decision-witness", "relationship", "duplicate"],
)
...
@pytest.mark.native
@pytest.mark.requires_native
def test_source_restore_and_public_record_replay_never_resurrect_withdrawn_decision(native):
    env, graph = native
    reference = env.references[2]
    dependency = env.dependencies[reference.document_id]
    project = KnowledgeService(env.database, env.identity).entity(
        env.scope,
        env.projects[0],
    )
    assert project.entity_type is not None and project.selection_witness is not None
    creation = RecordAuthoringRequest(
            ...
            entities=(
                AuthoredEntity(
                    ...
                    selection=CurrentSelection(
                        kind="current",
                        expected_entity_type=project.entity_type,
                        selection_witness=project.selection_witness,
                    ),
                ),
            ),
            assertions=(
                AuthoredAssertion(
                    ...
                    object=StringObject(
                        kind="string",
                        value="Withdraw this exact contribution",
                    ),
    ...
    created = graph.record(creation)
    target = created.receipt.assertions[0].stored_id
    assert graph.refresh().state == "ready"
    assert target in {
        member.decision.assertion_id
        for member in graph.relationship_decisions(request(env)).members
    }
    withdrawn = graph.write(withdrawal(env, target, writer="example"))
    assert withdrawn.status == "applied"
    assert graph.record(creation).receipt == created.receipt
    original = env.evidence.content(
        env.scope,
            ...
            WriteRequest(
                ...
                payload=PutDocument(
                    ...
                    metadata=SourceMetadata(
                        title="Synthetic note 2",
                        location="example://note/2",
                    ),
                ),
            )
        )
    ...
    assert target not in {member.decision.assertion_id for member in restored.members}
    ...
    replacement = graph.record(
        creation.model_copy(
            update={
                "request_id": "replacement",
                "retry_key": "replacement",
                "document": creation.document.model_copy(
                    update={
                        "support": {
                            "source": SourceCapture(
                                kind="source",
    ...
    replacement_id = replacement.receipt.assertions[0].stored_id
    assert replacement_id != target
    assert graph.refresh().state == "ready"
    final = graph.relationship_decisions(request(env))
    assert {member.decision.assertion_id for member in final.members} == {
        member.decision.assertion_id for member in restored.members
    } | {replacement_id}
...
def test_count_and_eof_share_original_generation(tmp_path, monkeypatch, mutation):
    ...
    env = fixture(
        ...
        decisions=0 if mutation == "empty-addition" else 12,
    )
    with LocalGraphSession(
        env.database,
        env.identity,
