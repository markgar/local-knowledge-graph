...
def query(env, project=None):
    return GraphTraversalRequest(
        request_id="withdrawal-query",
        scope=env.scope,
        start=GraphEntitySelector(entity_id=project or env.person),
        predicate="work:owns",
        direction="incoming" if project else "outgoing",
    )
    ...
    with LocalGraphSession(
        ...
        citations = tuple(
            StoredCitation(
                ...
                metadata_snapshot_id=e.metadata_snapshot_id,
...
@pytest.mark.native
@pytest.mark.requires_native
def test_public_record_replay_source_restore_and_new_assertion_do_not_resurrect(tmp_path):
    require_native()
    env = fixture(tmp_path / "canonical.sqlite", decisions=0)
    reference = env.references[2]
    dependency = env.dependencies[reference.document_id]
    knowledge = KnowledgeService(env.database, env.identity)
    person = knowledge.entity(env.scope, env.person)
    project = knowledge.entity(env.scope, env.projects[0])
    assert person.entity_type is not None and person.selection_witness is not None
    ...
    creation = RecordAuthoringRequest(
        ...
        document=RecordAuthoringDocument(
            ...
            entities=(
                AuthoredEntity(
                    local_id="person",
                    identity=ExistingIdentity(kind="existing", entity_id=env.person),
                    selection=CurrentSelection(
                        kind="current",
                        expected_entity_type=person.entity_type,
                        selection_witness=person.selection_witness,
                    ),
                ),
                AuthoredEntity(
                    local_id="project",
            ...
            assertions=(
                AuthoredAssertion(
                    ...
                    predicate="work:owns",
                    subject="person",
                    object=AuthoringEntityObject(kind="entity", entity="project"),
    ...
    created = knowledge.record(creation)
    ...
    with LocalGraphSession(
        ...
        before = graph.traverse(query(env))
        assert before.outcome == "complete"
        assert target in {path.assertion.assertion_id for path in before.paths}
        removed = graph.write(withdrawal(env, target))
        ...
        assert replayed.paths == tuple(
            path for path in before.paths if path.assertion.assertion_id != target
        )
        ...
        assert target not in {path.assertion.assertion_id for path in restored.paths}
        history = knowledge.contribution(env.scope, target, mode="history")
        assert history.withdrawal.withdrawal_id == removed.receipt.withdrawal_id
        ...
        replacement = graph.record(
            creation.model_copy(
                update={
                    "request_id": "new-assertion",
                    "retry_key": "new-assertion",
        ...
        assert {path.assertion.assertion_id for path in final.paths} == {
            path.assertion.assertion_id for path in restored.paths
        } | {replacement_id}
