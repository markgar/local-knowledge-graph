...
- Use the `work-package` skill in `.github/skills/work-package/SKILL.md` when preparing or resuming a package design. Follow `CONTRIBUTING.md`: write specs in session artifact storage outside the tracked repo, obtain independent critique and approval before coding, and preserve the approved spec on its package issue. Update the same artifact when the approved design changes.
