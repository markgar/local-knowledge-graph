...
class EntityInput:
    ...
    name: str
    entity_type: str
    support: SourceSupport | SeedSupport
...
class Fixture:
    ...
    def write(self, items):
        ...
        def source_support(value: SourceSupport) -> tuple[str, ...]:
            names = []
            for reference in value.evidence:
                key = reference.model_dump_json()
                name = source_names.setdefault(key, f"source-{len(source_names) + 1}")
                dependency = self.dependencies[reference.document_id]
                captures[name] = SourceCapture(
                    kind="source",
                    reference=reference,
                    state_version=dependency.state_version,
                )
                names.append(name)
            return tuple(names)
        ...
        def existing(entity_id: str) -> str:
            local_id = declared.setdefault(entity_id, f"stored-{len(declared) + 1}")
            if any(entity.local_id == local_id for entity in entities):
                return local_id
            view = KnowledgeService(self.database, self.identity).entity(self.scope, entity_id)
            if view.entity_type is None or view.selection_witness is None:
                raise RuntimeError("Fixture assertion endpoint is not classified")
            entities.append(
                AuthoredEntity(
                    local_id=local_id,
        ...
        for item in items:
            if isinstance(item, EntityInput):
                if isinstance(item.support, SeedSupport):
                    identity_name = f"{item.local_id}-identity"
                    classification_name = f"{item.local_id}-classification"
                    captures[identity_name] = SeedCapture(
                        kind="seed",
                        source_namespace=item.support.source_namespace,
                        seed_set_id=item.support.seed_set_id,
                        seed_key=item.support.seed_key,
                    )
                    captures[classification_name] = SeedCapture(
                        kind="seed",
                        source_namespace=item.support.source_namespace,
                        seed_set_id=item.support.seed_set_id,
                        seed_key=f"classification:{item.support.seed_key}",
                    )
                    identity = SeedSlotIdentity(
                        kind="seed_slot", name=item.name, support=identity_name
                    )
                    classification_support = classification_name
                else:
                    names = source_support(item.support)
                    identity = NewIdentity(kind="new", name=item.name, support=names)
                    classification_support = names
                entities.append(
                    AuthoredEntity(
            ...
            if not isinstance(item, AssertionInput):
                raise TypeError(f"Unsupported fixture item: {type(item).__name__}")
            subject = existing(item.subject)
            object_value = (
                AuthoringEntityObject(kind="entity", entity=existing(item.object_entity))
                if item.object_entity is not None
                else StringObject(kind="string", value=item.text or "")
            )
            assertions.append(
                AuthoredAssertion(
                    local_id=item.local_id,
                    ...
                    object=object_value,
                    interpretation="explicit",
                    support=source_support(item.support),
                ),
            )
        result = KnowledgeService(self.database, self.identity).record(
            RecordAuthoringRequest(
        ...
        mapping = {
            **{item.local_id: item.entity_id for item in result.receipt.entities},
            **{item.local_id: item.stored_id for item in result.receipt.assertions},
        }
        self.selections.update(
            {
                item.entity_id: item.selection_id
                for item in result.receipt.entities
                if item.selected_claim_id is not None
            }
        )
        for item in items:
            if isinstance(item, AssertionInput):
                self.expected[mapping[item.local_id]] = {
                    "subject": item.subject,
                    "object": item.object_entity,
                    "text": item.text,
                    "support": item.support.evidence,
                }
        return mapping
...
def fixture(path: Path, *, decisions: int = 12, varied: bool = False) -> Fixture:
    ...
    registered = EvidenceAdministration(database, authority).register(
        CorpusRegistration(
            corpus_id="graph-example",
        ...
        SchemaPresetRequest(
            ...
            definition=SchemaDefinition(
                ...
                predicates=(
                    SchemaPredicateDefinition(
                        name="work:owns",
                        description="The subject is accountable for the object.",
                    ...
                    SchemaPredicateDefinition(
                        name="work:decision",
                        description="An explicit decision about the subject.",
    ...
    attribution = Attribution(
        owner_id="owner",
        writer_id="example",
        producer="synthetic-graph-example",
        producer_version="1",
    )
    env = Fixture(
        database,
            ...
            WriteRequest(
                ...
                payload=PutDocument(
                    ...
                    document=ExternalDocument(
                        source_namespace=ns, synchronization_scope="local", external_id=f"note-{i}"
                    ),
                    ...
                    metadata=SourceMetadata(
                        title=f"Synthetic note {i}", location=f"example://note/{i}"
                    ),
                ),
            )
        )
    ...
    for i in range(project_count):
        if i % 5 == 0:
            support = SeedSupport(
                ...
                seed_set_id="projects",
                seed_key=f"project-{i}",
            )
        else:
            index = (i * 2 + 1) if i % 5 in (1, 2) else i * 2
            support = SourceSupport(
                kind="source", evidence=(env.references[index % len(env.references)],)
            )
        entity_id = env.write(
            (
                EntityInput(
    ...
    for i in range(decisions):
        ...
        batch.append(
            AssertionInput(
                local_id=f"decision-{i}",
                predicate="work:decision",
                subject=env.projects[project],
                object_entity=None,
                text="Independently submitted same-text decision",
                support=SourceSupport(kind="source", evidence=refs),
            )
        )
...
def verify(stage, operation, env):
    ...
    with stage.observer.operation(
        ...
        env.identity,
        env.scope,
        operation.budget.deadline,
        operation.budget,
    ) as current:
        with current.read_context(operation.meter) as context:
            for identifier, payload in query(
                stage.native,
                operation,
                ...
                for ordinal, proof in enumerate(actual.support, 1):
                    assert (
                        proof.captured.dependency
                        == env.dependencies[proof.captured.reference.document_id]
                    )
            ...
            for row in query(
                ...
                ref = EvidenceRef(
                    ...
                    source_namespace=data["namespace"],
                ...
                proof = GraphEvidence(
                    captured=CapturedEvidence(
                        reference=ref,
                        dependency=env.dependencies[ref.document_id],
            ...
            proofs = dict(
                query(
                    stage.native,
                    operation,
                    "MATCH (e:Entity)-[:SELECTED]->(p:EntityProof) "
                    "RETURN e.entity_id,p.witness_json",
                )
            )
            ...
            expected_objects = {
                ...
                for identifier, expected in env.expected.items()
                if expected["object"] is not None
            }
            ...
            expected_topology = {
                ...
                "OBJECT": [
                    (key, e["object"], 0)
                    for key, e in env.expected.items()
                ...
                if isinstance(witness.basis, SeedWitness):
                    ...
                    for ordinal, captured in enumerate(witness.basis.evidence, 1):
                        source_expected.append(
                            (
                                proof_id(witness),
                                captured.reference.document_id,
                                captured.reference.anchor_id,
                                captured.reference.passage_id,
                                ordinal,
                                captured.dependency.state_version,
                                captured.metadata_snapshot_id,
            ...
            assert Counter(
                query(
                    ...
                    "e.metadata_snapshot_id,e.namespace_token",
                )
            ) == Counter(source_expected)
            assert Counter(
                query(
                    stage.native,
                    operation,
                    "MATCH (p:EntityProof)-[:ENTITY_SEED]->(s:SeedProof) "
                    "RETURN p.proof_id,s.proof_id,"
                    "s.namespace,s.owner_id,s.writer_id,s.seed_set_id,s.seed_key,s.contribution_id,"
                    "s.membership_event_id,s.generation",
                )
            ) == Counter(seed_expected)
...
def run(output: Path, *, varied: bool):
    ...
    try:
        stage = build_graph(
            env.database, env.identity, env.scope, staging_parent=output, operation=operation
        )
        ...
        result = {
            ...
            "manifest": stage.manifest.model_dump(mode="json"),
        ...
        result = {
            ...
            "accounting": asdict(error.accounting),
            "cleanup": asdict(error.cleanup),
        }
