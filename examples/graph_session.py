...
def lifecycle(output: Path) -> dict:
    ...
    session = timed("open", lambda: LocalGraphSession(
    ...
    try:
        ...
        def proof(context):
            ...
            rows.close()
            evidence = tuple(context.retain(read_evidence(
                context.canonical, item.captured.reference,
        ...
        batch = WriteBatch(
            contract_version="foundation/1", batch_id=str(uuid4()),
            items=(request, request.model_copy(update={
                "request_id": str(uuid4()), "retry_key": str(uuid4()),
            })),
        )
        written = timed("batch_write", lambda: session.write_batch(batch))
        ...
        assert env.evidence.write(request).receipt == receipt.receipt
        ...
        with LocalGraphSession(
            ...
            assert timed(
                "restart_build_read", lambda: restarted._run_read(env.scope, native_count),
            ) == 24
            assert restarted._generation.session_id != generation.session_id
        result = {
            "cold_assertions": first, "warm_assertions": second,
            "refreshed_assertions": refreshed, "saved_receipt_after_graph_failure": True,
            "canonical_proof_hydrated": True, "restart_untrusted": True,
            "native_buffer_bytes": BUFFER_POOL_BYTES, "close": asdict(session.status()),
