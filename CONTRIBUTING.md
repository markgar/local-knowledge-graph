...
`evidence-store/3`. Ladybug holds only rebuildable, exact-scope derived data.
...
| Path | Responsibility |
| --- | --- |
| `src/kg/evidence/`, `src/kg/evidence/schema.sql` | Canonical `evidence-store/3`, exact supplied-source intake/history, trusted local policy and owner transactions. |
...
| `src/kg/schema.sql`, `src/kg/db.py` | Separate Markdown demonstration SQLite schema and connections, not `evidence-store/3`. |
